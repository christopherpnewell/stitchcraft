...
export async function generatePdf(pattern) {
  return new Promise((resolve, reject) => {
    ...
    const doc = new PDFDocument({
      ...
      info: {
        Title: `Loominade — ${projectInfo.label} Pattern`,
        Author: 'Loominade',
...
function drawTitlePage(doc, pattern, projectInfo, totalPages) {
  ...
  doc.fontSize(28).font('Helvetica-Bold')
    .text('Loominade', PAGE.marginLeft, PAGE.marginTop, {
      width: CONTENT_WIDTH,
      align: 'center',
    });
  ...
  for (const color of pattern.palette) {
    if (sx + swatchSize + 50 > PAGE.marginLeft + CONTENT_WIDTH - 20) {
      sx = leftCol;
      y += swatchSize + 8;
    }
    ...
    doc.fontSize(8).font('Helvetica').fillColor('#333')
      .text(color.label, sx + swatchSize + 3, y + 2, { width: 40 });
    sx += swatchSize + 32 + swatchGap;
  }
...
function drawLegendPage(doc, pattern, pageNum, totalPages) {
  ...
  for (let i = 0; i < pattern.palette.length; i++) {
    ...
    doc.fontSize(9).font('Helvetica-Bold').fillColor('#000')
      .text(sym, cols.symbol, rowY + 6, { width: 25, lineBreak: false });
  ...
  // Notes section
  doc.moveDown(1);
  doc.fontSize(10).font('Helvetica-Bold').fillColor('#000').text('Notes');
  doc.moveDown(0.5);
  ...
  doc.moveDown(0.15);
  doc.text('• This pattern was generated by Loominade. For best results, swatch with your chosen yarn to confirm gauge.');
  doc.moveDown(0.15);
  ...
  if (pattern.floatAnalysis?.hasLongFloats) {
    doc.moveDown(0.15);
    doc.fontSize(9).font('Helvetica-Bold').fillColor('#c00');
