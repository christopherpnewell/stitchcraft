...
function drawTitlePage(doc, pattern, projectInfo) {
  ...
  const instructions = [
    ...
    '• Row 1 is at the bottom of the chart. Read every row left to right as displayed (modern convention).',
...
function drawLegendPage(doc, pattern) {
  ...
  doc.moveDown(0.5);
  doc.fontSize(8).font('Helvetica').fillColor('#555');
  doc.text('• Yardage estimates include a 15% safety buffer and are scaled to your selected yarn weight.');
...
function getYardsPerStitchPdf(stitchGauge) {
  if (stitchGauge <= 14) return 0.032;
  if (stitchGauge <= 20) return 0.022;
  if (stitchGauge <= 24) return 0.018;
  if (stitchGauge <= 26) return 0.015;
  return 0.012;
}
